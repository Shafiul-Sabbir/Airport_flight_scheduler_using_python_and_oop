...
class TravelAgent:
    def __init__(self, name) -> None:
        ...
        """ 
            ...
            start date : date when you want to start the trip
    ...
    def set_trip_one_city_one_way(self, start, end, start_date):
        price = self.all_airport.get_ticket_price(start, end)
        distance = self.all_airport.distance_between_two_airports(start, end)
    ...
    """ 
        trip_info: [start = city1, end = city2, again return to = city1]
    """
    ...
    def set_trip_two_city_one_way(self, trip_info, start_date):
        trip1 = self.set_trip_one_city_one_way(trip_info[0], trip_info[1], '24/08/23')
        trip2 = self.set_trip_one_city_one_way(trip_info[1], trip_info[2], '26/08/23')
        return [trip1, trip2]
    ...
    def set_trip_multi_city_one_way_fixed_route(self, trip_info, start_date):
        ...
        for i in range(0, len(trip_info) - 1):
            trip = self.set_trip_one_city_one_way(trip_info[i], trip_info[i+1], '24/08/23')
            trips.append(trip)
        return trips
    ...
    def set_trip_multi_city_flexible_route(self, trip_cities, start_date):
        ...
        # print(start_city, flexible_cities)
        min_price = float('inf')
        selected_trip = None
